...
function useThemeStyles(styleFeatures, systemTheme, setStyleFeatures, loggedInUser) {
    ...
    useEffect(() => {
        const handleColorPickerChange = () => {
            const checkForRootElement = () => {
                ...
                if ((isLightTheme || isSystemLight) && styleFeatures.color.primaryTextLight === '#FFFFFF') {
                    ...
                    root.style.setProperty('--cometchat-text-color-primary', isLightTheme || isSystemLight ? styleFeatures.color.primaryTextLight : styleFeatures.color.primaryTextDark);
                }
                ...
                if ((isLightTheme || isSystemLight) && styleFeatures.color.secondaryTextLight === '#989898') {
                    ...
                    root.style.setProperty('--cometchat-text-color-secondary', isLightTheme || isSystemLight
        ...
        const handleFontSizeChange = () => {
            const selectedFontSize = fontSizes[styleFeatures.typography.size] || {};
    ...
    useEffect(() => {
        ...
        const recolorCanvasContent = (canvas) => {
        ...
        const findAndRecolorCanvases = (element) => {
            if (element instanceof Element && element.matches('canvas')) {
                recolorCanvasContent(element);
            }
            // Search within child elements and Shadow DOM recursively
